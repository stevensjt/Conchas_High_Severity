...
xmax <- 200
ymax <- 200
#extent(perim_sf)@xmax[[1]] depricate
extent(perim_sf)@xmax[[1]] - extent(perim_sf)@xmin[[1]] 
...
pct <- 5 #Key parameter to vary autocorrelation (JTS)
...
####2. Conchas application####
#### Load Las Conchas specific files###
perim <- readOGR("../../../GIS/Fires/Las Conchas/Perimeter/", layer="Las_Conchas_Perim")
perim_sf <- as(perim,"sf")
#Possible transformation: newData<-spTransform(x, CRS("+init=epsg:4238"))
...
# copy the binary grid and convert the zeros to NAs  
gf.binary1 <- gf.binary.mask #Specify whether to calculate clipped to perim ("mask") or not.
gf.binary1[gf.binary != 1] <- NA
